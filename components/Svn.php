...
class Svn extends Command {
    ...
    public function updateRepo($branch = 'trunk', $svnDir = null, $taskId = '') {
     ...
     * @return bool
     */
    public function updateToVersion($task, $taskId = '') {
        ...
        $command = join(' && ', $cmd);
        return $this->runLocalCommand($command, $task->id);
    }
    ...
    public function getFileBetweenCommits($branch, $star, $end) {
        ...
        $files = array_map(function($item) {
            if(substr($item,0, strpos($item, " ")) != 'D'){
