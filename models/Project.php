 ...
 * @property string $release_excludes
...
class Project extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['excludes', 'hosts', 'release_excludes', 'pre_deploy', 'post_deploy', 'pre_release', 'post_release', 'hg_deploy'], 'string'],
            ...
            [['release_user', 'repo_mode', 'repo_username', 'repo_port'], 'string', 'max' => 50],
    ...
    public static function getDeployFromDirFtp(){
        $from    = static::$CONF->deploy_from;
        $env     = isset(static::$LEVEL[static::$CONF->level]) ? static::$LEVEL[static::$CONF->level] : 'unknow';
        $project = static::getGitProjectName(static::$CONF->repo_url);

        return sprintf("%s/%s", rtrim($from, '/'), rtrim($env, '/'));
    }
   ...
   /*获取回滚命令字符串*/
    public static function getProjectCommand() {
        return static::$CONF->hg_deploy;
    }
    ...
    /**
     * 拼接目标机webroot的目录
     * {release_to}/{project}/{version}
     *
     * @param $version
     * @return string
     */
    public static function getWebrootVersionDir($version = ''){
        return sprintf('%s/%s', rtrim(static::$CONF->release_to, '/'),static::getGitProjectName(static::$CONF->repo_url));
    }
