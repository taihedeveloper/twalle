...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function scenarios()
    {
        return array_merge(parent::scenarios(), [
            'signup' => ['username','email','password','role','realname','group_id'],
        ]);
    }
    ...
    {
        return [
            [['username','email', 'realname', 'password','group_id'], 'required', 'on'=>'signup'],

            ['status', 'default', 'value' => self::STATUS_ACTIVE],
            ['status', 'in', 'range' => [self::STATUS_ACTIVE, self::STATUS_INACTIVE]],

            ['role', 'default', 'value' => self::ROLE_DEV],
            ['role', 'in', 'range' => [self::ROLE_USER, self::ROLE_DEV, self::ROLE_ADMIN,self::SUPER_ADMIN]],
    ...
    public static function getUsername($user_ids) {
        if(empty($user_ids)){
            return ;
        }
        $p_ids = implode(",", $user_ids);
        $sql = "select id,username from `user` where id in(".$p_ids.")";
