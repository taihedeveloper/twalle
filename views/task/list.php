...
<div class="box">
        ...
        <form action="/task/" method="POST">
            ...
            <div class="col-xs-12 col-sm-8" style="padding-left: 0;margin-bottom: 10px;">
                <div class="input-group">
                    <input type="text" name="kw" class="form-control search-query" placeholder="<?= yii::t('task', 'list placeholder') ?>">
    ...
    <div class="box-body table-responsive no-padding clearfix">
		...
		<th>审核人</th>
			...
			<a href="<?= Url::to("@web/task/get-file-list/?projectID={$item['id']}") ?>" data-toggle="modal" data-target="#myModal">
                                <i class="icon-zoom-in bigger-130"></i>
                                <?= yii::t('task', '详细信息') ?>
                        </a>

			
			<a href="<?= Url::to("@web/task/download-log/?projectID={$item['id']}") ?>" target="_blank">
		...
		<td><?php $pid = $item['project_id']; if($manager[$pid]){echo implode(',',$manager[$pid]);}?></td>
            </tr>
            <?php } ?>
